...
The Worker is deployed at `https://anestesiahelppi-chat.koponenkasperi8.workers.dev`. The original inactive `/health` check succeeded on 2026-09-19. The user added the Groq secret directly in Cloudflare. The research abstract demo is now configured with `CHAT_ENABLED=true` and `RESEARCH_ENABLED=true`; see [research-demo.md](research-demo.md) for this separate evidence mode and verification status. The reviewed clinical corpus remains empty.

Cloudflare Builds watches `feat/cloudflare-chat` with root directory `cloudflare`, build command `npm ci`, and deploy command `npm run deploy`. GitHub Pages continues to build from `main`.
...
The deploy command prints the actual `https://anestesiahelppi-chat.<account-subdomain>.workers.dev` URL. Do not guess the account subdomain. Open `/health` on that URL: it should report `status: ok` and `chatEnabled: false`. For an inactive deployment, set `CHAT_ENABLED=false` before deploying. A deployment creates one SQLite-backed Durable Object namespace, available on the free plan, for persistent quota state.
...
The Pages workflow now provides the public research endpoint by default. To restore a demo-only frontend, remove that workflow fallback and the repository variable, then rebuild. Set `CHAT_ENABLED=false` and redeploy to disable the backend. No provider billing or paid fallback is enabled by this implementation.

## Quotas and privacy

One named Durable Object (`global-demo-v1`) handles all quota reservations inside storage transactions. The limit is 50 accepted requests per UTC day, at least 60 seconds apart. Reservations are persisted before inference and are not refunded on provider errors, preventing retry storms and quota races. Failure to reach quota storage blocks inference. Only date, count and next-permitted time are stored; questions, answers and API keys are not stored in the object.
